...
<fmt:setBundle basename="sunmin.jdbc" />
...
<sql:query dataSource="${mariadb}" var="rs">
  ...
  <h2>게시판</h2>
     <table>
         ...
         <thead>
             <tr><td colspan="5" class="newbtn">
              <c:if test="${not empty sessionScope.userid}">
              	<button type="button" id="newwritebtn">글쓰기</button>
              </c:if>
      ...
      <script>
        	var newwritebtn = document.getElementById('newwritebtn');
        	newwritebtn.addEventListener('click', go2write);
